...
exports.createQuiz = async (req, res) => {
  try {
    ...
    const validQuestions = questions.every(q =>
      q.questionText && q.options?.length === 4 && q.options.includes(q.correctAnswer)
    );

    if (!validQuestions) {
      return res.status(400).json({
        message: 'Each question must have questionText, 4 options, and correctAnswer among options'
      });
    }
